    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Card className="bg-blue-50">
        <CardContent className="p-4">
          <h4 className="font-medium text-blue-900 mb-2">وضعیت انرولمنت چهره</h4>
          <div className="text-sm text-blue-700">
            {person.faceEnrollments && person.faceEnrollments.length > 0 ? (
              <div>
                <p>✓ چهره این شخص انرول شده است</p>
                <p>تعداد انرولمنت‌ها: {person.faceEnrollments.length}</p>
                <p>آخرین انرولمنت: {new Date(person.faceEnrollments[0].createdAt).toLocaleDateString('fa-IR')}</p>
              </div>
            ) : (
              <p>⚠ چهره این شخص هنوز انرول نشده است</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gray-50">
        <CardContent className="p-4">
          <h4 className="font-medium text-gray-900 mb-2">آمار تردد</h4>
          <div className="text-sm text-gray-600 space-y-1">
            <p>تعداد کل ترددها: {person.attendanceLogs?.length || 0}</p>
            <p>تاریخ ایجاد: {new Date(person.createdAt).toLocaleDateString('fa-IR')}</p>
            <p>آخرین به‌روزرسانی: {new Date(person.updatedAt).toLocaleDateString('fa-IR')}</p>
          </div>
        </CardContent>
      </Card>
